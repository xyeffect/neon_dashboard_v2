...
# Feature 1: Drop Rate (recalculated, Archived excluded)
...
print("Rule-based tier distribution:")
...
preds = preds.merge(
    risk[["School Name", "survival_rate", "risk_final"]],
...
# Use New_Course_Allocations.csv directly to avoid column name conflicts
ca_fresh = pd.read_csv("New_Course_Allocations.csv")
...
# Priority 1: use Seat Goal from course.csv directly
# Priority 2: if Seat Goal is NaN for a specific term, use the median
#             Seat Goal for that course across all other terms
# Priority 3: if course is entirely missing from course.csv,
#             use the median Seat Goal across all courses of similar size
...
# Fill NaN Seat Goals within the same course using median across other terms
course_median_goal = (
...
# Fill NaN Seat Goal with course-level median
nan_goal = all_ct["Seat Goal"].isna()
all_ct.loc[nan_goal, "Seat Goal"] = all_ct.loc[nan_goal, "median_seat_goal"]

# For courses entirely missing from course.csv, use global median
...
n_filled = nan_goal.sum()
n_missing = still_missing.sum()
if n_filled:
    print(f"{n_filled} course-term rows: Seat Goal filled from course median")
if n_missing:
    print(f"{n_missing} course-term rows: Seat Goal filled from global median ({global_median:.0f})")
...
def compute_threshold(group, seat_goal):
    ...
    # Accepted CA threshold: how many accepted CAs needed to hit Seat Goal
    accepted_threshold = np.ceil(seat_goal / avg_expected) if avg_expected > 0 else np.nan

    # Old threshold: program-level average, no risk adjustment
    old_threshold = np.ceil(seat_goal / PROGRAM_AVG)

    # Offer threshold: how many CAs NEON needs to offer to hit Seat Goal
    # Uses tier-specific conversion rate weighted by active CAs in this course
    if n_active > 0:
        active = active.copy()
...
print(f"  Squatting: {flags['flag_squatting'].sum()}")
